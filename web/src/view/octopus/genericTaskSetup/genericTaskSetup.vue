...
<script setup>
import {
  createGenericTaskSetup,
  deleteGenericTaskSetup,
...
const getTableData = async () => {
  const table = await getGenericTaskSetupList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    deleteGenericTaskSetupFunc(row)
  })
}
...
const onDelete = async () => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    const res = await deleteGenericTaskSetupByIds({ IDs })
    if (res.code === 0) {
      ElMessage({
        type: 'success',
        message: '删除成功'
...
// 更新行
const updateGenericTaskSetupFunc = async (row) => {
  const res = await findGenericTaskSetup({ ID: row.ID })
  type.value = 'update'
  if (res.code === 0) {
...
// 删除行
const deleteGenericTaskSetupFunc = async (row) => {
  const res = await deleteGenericTaskSetup({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
const enterDialog = async () => {
  elFormRef.value?.validate(async (valid) => {
    ...
    switch (type.value) {
      case 'create':
        res = await createGenericTaskSetup(formData.value)
        break
      case 'update':
        res = await updateGenericTaskSetup(formData.value)
        break
      default:
        res = await createGenericTaskSetup(formData.value)
        break
