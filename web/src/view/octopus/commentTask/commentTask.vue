  ...
  :disabled-date="time=> searchInfo.startCreatedAt ? time.getTime() < searchInfo.startCreatedAt.getTime() : false"></el-date-picker>
    ...
    <div class="gva-table-box">
      ...
      <el-table ref="multipleTable" style="width: 100%" tooltip-effect="dark" :data="tableData" row-key="ID"
        ...
        <el-table-column align="center" label="任务标题" prop="title" width="180" />
...
const getTableData = async () => {
  const table = await getCommentTaskList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async () => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    const res = await deleteCommentTaskByIds({ IDs })
    if (res.code === 0) {
      ElMessage({
        type: 'success',
        message: '删除成功'
...
// 更新行
const updateCommentTaskFunc = async (row) => {
  const res = await findCommentTask({ ID: row.ID })
  type.value = 'update'
  if (res.code === 0) {
...
// 删除行
const deleteCommentTaskFunc = async (row) => {
  const res = await deleteCommentTask({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
